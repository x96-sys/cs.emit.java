...
class EmitTermTest {
    ...
    void happy(){
        ...
        assertEquals("ceci+", new EmitTerm(new Term(false, new Identity("ceci".getBytes()), Optional.of(Occurrence.OneOrMore))).toCS());
        assertEquals("!ceci?", new EmitTerm(new Term(true, new Identity("ceci".getBytes()), Optional.of(Occurrence.ZeroOrOne))).toCS());
